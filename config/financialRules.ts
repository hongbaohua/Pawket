...
// 一次對帳只支援「一份月結單」等級的資料量，避免重演「整份歷史明細丟給AI
// 結果漏掉大半資料」的問題(2026-07-26踩過：2.5年791筆一次送出，AI只回傳210筆)。
// 2026-09-07：原本用「頁數」當資料量的替代指標(超過N頁就擋)，Ivy反應這樣不合理——
// 頁數只是排版產物，同樣一個月的消費，字體/欄寬/銀行格式不同，頁數就會不一樣，
// 這次她單月4頁的正常月結單就被3頁的門檻誤擋。改成直接從抽出來的文字裡數「看起來
// 像日期」的樣式(ReconcileView.tsx的estimateTransactionRowCount)當資料量指標，
// 這才是真正影響AI會不會漏資料的數字，不受排版影響——正常一個月即使消費很密集，
// 抓到的日期樣式數也不會逼近2.5年791筆那種數量級。
export const RECONCILE_MAX_ESTIMATED_ROWS = 300;
...
// 喵喵心願罐「接下來的計畫」（2026-09-08）：Ivy要求還差多少的心願要能推算「照現在
// 的存錢速度，大概什麼時候買得起」，跟她確認過用「最近幾個月平均淨現金流」當存錢
// 速度的基準——選最近6個月(比3個月穩定、不會被單月異常大支出/獎金拖累太多，比12個月
// 更能反映近期真實狀況)。
export const WISHLIST_PACE_LOOKBACK_MONTHS = 6;
