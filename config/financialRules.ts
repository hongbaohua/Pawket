...
// 2026-08-26 Ivy反應：日均燒錢速度應該先排除極端值——單筆罕見的大額支出（例如一次繳一整年
// 的保費、買一台筆電）如果直接算進日均，會讓「還能撐幾天」被單一一筆特殊支出嚴重低估，
...
// 明顯斷層的那幾筆(例如年繳保費$47,515)，$2640以下的正常大額支出不受影響
// （Ivy確認要「更保守的統計倍數」，而不是改用固定金額門檻或只排除前1~2筆）。
